...
def render_demo_html(data):
    """Render a client demo without Python f-string/HTML brace conflicts."""
    ...
    excluded = {
        'hero', 'reviews', 'contact', 'faq', 'why choose us', 'service area',
        'about', 'trust', 'gallery'
    }
    sections = [str(x) for x in (brief.get('recommended_sections') or [])]
    ...
    seo_focus = [str(x) for x in (brief.get('seo_focus') or [])][:6]
    ...
    if rating is not None:
        trust.append('<div class="trust-item"><strong>★ ' + esc(rating) + '</strong><span>Google rating</span></div>')
    if reviews is not None:
        trust.append('<div class="trust-item"><strong>' + esc(reviews) + '+</strong><span>Google reviews</span></div>')
    trust.append('<div class="trust-item"><strong>Local</strong><span>Service-focused</span></div>')
    trust_html = ''.join(trust)

    service_html = ''.join(
        '<article class="service-card"><div class="icon">✓</div><h3>' + esc(s) + '</h3>'
        '<p>Request current availability, pricing, and service details from ' + esc(name) + '.</p>'
        '<a href="#quote">Request service →</a></article>' for s in services
    )

    gallery_html = ''.join(
        '<figure><img src="' + esc(p) + '" alt="' + esc(name) + ' business photo" loading="lazy">'
        '<figcaption>Verified business photo</figcaption></figure>' for p in photos
    )
    ...
    if hero_image:
        hero_style = "background-image:linear-gradient(90deg,rgba(4,13,32,.96),rgba(4,13,32,.72),rgba(4,13,32,.30)),url('" + esc(hero_image) + "')"
    else:
        hero_style = 'background-image:linear-gradient(135deg,#07152f,#123b72,#0a1b38)'
...
<meta name="description" content="__NAME__ — __CATEGORY__ serving __CITY__. Request service or a quote today.">
<meta name="theme-color" content="#07152f">
<title>__NAME__ | __CATEGORY__</title>
...
<body>
<div class="topbar"><b>Need service?</b> Call now for a faster response.</div>
<nav><div class="brand">__NAME__</div><div class="navlinks"><a href="#services">Services</a><a href="#why">Why Us</a><a href="#reviews">Reviews</a><a href="#area">Service Area</a></div><a class="btn btn-gold" href="__PHONE_LINK__">__NAV_CTA__</a></nav>
<header class="hero" style="__HERO_STYLE__"><div class="hero-inner"><div class="eyebrow">__CATEGORY__ · __CITY__</div><h1>Reliable __CATEGORY_LOWER__ when you need it.</h1><p>Get clear information, responsive communication, and a simple way to request service from <strong>__NAME__</strong>.</p><div class="hero-actions"><a class="btn btn-gold" href="#quote">__CTA__</a><a class="btn" style="background:#fff;color:#0a1730" href="__PHONE_LINK__">__CALL_CTA__</a></div><div class="hero-note">Local-service website designed around calls, quote requests, trust, and location relevance.</div></div></header>
<main>
<div class="wrap"><div class="trust-strip">__TRUST__</div></div>
<section id="services" class="services"><div class="wrap"><div class="section-head"><h2>Services customers can request</h2><p>Choose the service you need and request current availability or pricing directly.</p></div><div class="service-grid">__SERVICES__</div></div></section>
<section id="why" class="why"><div class="wrap why-grid"><div><div class="section-head"><h2>A better local-service experience</h2><p>The page keeps the most important conversion paths visible while making the business, service, and location context easy to understand.</p></div><div class="checklist"><div class="check"><b>✓</b><span>Clear service-first navigation</span></div><div class="check"><b>✓</b><span>Prominent call and quote actions</span></div><div class="check"><b>✓</b><span>Verified trust information</span></div><div class="check"><b>✓</b><span>Mobile sticky conversion bar</span></div></div></div><div class="why-box"><h3>Need help now?</h3><p>Use the fastest available contact option below to request service.</p><a class="btn btn-gold" href="__PHONE_LINK__">__CALL_CTA__</a></div></div></section>
<section id="reviews"><div class="wrap"><div class="section-head"><h2>Local trust at a glance</h2><p>Only verified Google Business Profile rating and review-count data supplied by the lead are shown.</p></div><div class="review-box">__REVIEW__ <a class="btn btn-outline" href="__MAPS_LINK__" target="_blank" rel="noopener">View on Google Maps</a></div></div></section>
__GALLERY_SECTION__
<section id="area" class="area"><div class="wrap"><div class="area-card"><div><h2>Serving __SERVICE_AREA__</h2><p>__ADDRESS__</p></div><a class="btn btn-blue" href="__MAPS_LINK__" target="_blank" rel="noopener">Open Google Maps</a></div></div></section>
__OPPORTUNITIES__
<section id="quote" class="quote"><div class="wrap quote-grid"><div><div class="eyebrow">Request service</div><h2>Ready to get started?</h2><p>Send a short request. The business can follow up with availability, pricing, and next steps.</p><p><strong>__NAME__</strong><br>__ADDRESS__<br>__PHONE__</p></div><form class="form" onsubmit="submitLead(event)"><div class="form-grid"><div><label for="fullName">Name</label><input id="fullName" required placeholder="Your name"></div><div><label for="phoneField">Phone</label><input id="phoneField" required placeholder="(555) 555-5555"></div><div class="full"><label for="serviceField">Service needed</label><input id="serviceField" placeholder="What do you need help with?"></div><div class="full"><label for="messageField">Message</label><textarea id="messageField" placeholder="Tell us briefly what you need."></textarea></div><div class="full"><button class="btn btn-gold" type="submit">Request a Quote</button><p id="formStatus" style="font-size:12px;color:#60708b;margin:10px 0 0">Your request is prepared for the business to follow up.</p></div></div></form></div></section>
...
__SEO_STRIP__
<footer><div class="footer-grid"><div><strong>__NAME__</strong><br>__CATEGORY__ · __CITY__</div><div>__ADDRESS__<br>__PHONE__</div></div></footer>
...
<script>
function submitLead(e){e.preventDefault();var name=document.getElementById('fullName').value.trim();var phone=document.getElementById('phoneField').value.trim();var service=document.getElementById('serviceField').value.trim();var msg=document.getElementById('messageField').value.trim();var text='Name: '+name+'\\nPhone: '+phone+'\\nService: '+service+'\\nMessage: '+msg;document.getElementById('formStatus').textContent='Request captured. Please use the phone/contact method above to send it to the business.';window.location.href='mailto:?subject='+encodeURIComponent('Service request for __NAME__')+'&body='+encodeURIComponent(text)}
</script>
</body></html>'''

    review_html = ''
    if rating is not None and reviews is not None:
        review_html = '<div class="stars">★★★★★</div><h3>Google rating: ' + esc(rating) + '</h3><p class="review-meta">Based on ' + esc(reviews) + ' Google reviews.</p>'
    else:
        review_html = '<h3>Google review data not supplied</h3><p class="review-meta">Connect verified Business Profile data to display live rating and review count.</p>'

    gallery_section = ''
    if gallery_html:
        gallery_section = '<section><div class="wrap"><div class="section-head"><h2>Real business photos</h2><p>Photos supplied by the verified lead source are displayed without inventing business imagery.</p></div><div class="gallery-grid">' + gallery_html + '</div></div></section>'

    opportunities_section = ''
    if opp_html:
        opportunities_section = '<section><div class="wrap"><div class="section-head"><h2>Growth opportunities</h2><p>Generated from the selected lead and demo brief.</p></div><ul class="checklist">' + opp_html + '</ul></div></section>'

    seo_section = ''
    if focus_html:
        seo_section = '<div class="seo-strip"><div class="seo-inner"><strong>SEO focus:</strong>' + focus_html + '</div></div>'

    values = {
        '__NAME__': esc(name),
        '__CATEGORY__': esc(category),
        '__CATEGORY_LOWER__': esc(category.lower()),
        '__CITY__': esc(city or 'Local Service Area'),
        '__ADDRESS__': esc(address or 'Contact the business to confirm the service location.'),
        '__PHONE__': esc(phone),
        '__PHONE_LINK__': phone_link,
        '__NAV_CTA__': 'Call Now' if phone else 'Get a Quote',
        '__CALL_CTA__': 'Call ' + esc(phone) if phone else 'Contact Business',
        '__MOBILE_CALL__': 'Call Now' if phone else 'Contact',
        '__CTA__': esc(cta),
        '__HERO_STYLE__': hero_style,
        '__SCHEMA__': json.dumps(schema, ensure_ascii=False, separators=(',', ':')),
        '__TRUST__': trust_html,
        '__SERVICES__': service_html,
        '__REVIEW__': review_html,
        '__GALLERY_SECTION__': gallery_section,
        '__SERVICE_AREA__': esc(city or address or 'your local area'),
        '__MAPS_LINK__': esc(maps_link),
        '__OPPORTUNITIES__': opportunities_section,
        '__SEO_STRIP__': seo_section,
    }
